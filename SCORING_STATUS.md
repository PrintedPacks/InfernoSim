...
| `healerReach` | **Live** (2026-08) | 1 if the **focus healer** — nearest Jad healer still healing (`aggro !== player`) — can be tagged from the destination at blowpipe reach (`focusHealer`). The Jad-wave analogue of `barrageReach`: healers cluster on Jad's far side, and the tag-and-turn depends on standing where a blowpipe reaches them. Same under-mob refusal as reach; null focus on every other wave, so it costs nothing outside 67+ |
...
| `damageTaken` | **Live** | Full 12-tick (`HORIZON_TICKS`) simulation of the walk + best possible prayer plan against it; what's left over. **Two exceptions, both 2026-08:** Jad's landings are priced by `OverheadPlanner` as **0 if the plan shows Magic or Range (either), full max hit only if a genuine same-tick collision forces the slot elsewhere** — not the mager/ranger-style 50/50 average — because `JadTracker` observes Jad's *real* committed style before it lands, so a live prayer choice of Magic/Range is never a guess, only the static tile-scoring sim (which never predicts the roll) still sees it as ambiguous. The old 50%-per-landing pricing put a flat ~−226 floor on every tile in the arena (Jad's range is 50), swamping every real positional signal — measured, a wave-68 dump where the whole grid scored within a few points of that floor. **Healer hits are priced only if they land on the very next tick** (`priced` filter in `scoreRoute`) — a projected catch-up 8 ticks out is a guess, not a fact, and pricing it sent the bot sprinting real distance every tick to buy a temporary reprieve it re-bought the next tick anyway, at the cost of Jad DPS uptime. `parts.threats` now reports the *priced* count, not the raw one, so the dump's threat column matches what was actually charged |
