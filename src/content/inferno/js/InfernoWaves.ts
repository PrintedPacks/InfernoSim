...
export class InfernoWaves {
  ...
  static spawnNibblers(n: number, region: Region, pillar: Entity) {
    const mobs: Mob[] = [];
    // InfernoWaves.shuffle, not lodash's. Lodash captures `Math.random` into a local at module
    // load time, so it cannot be seeded from outside and these nine tiles came out in a different
    // order on every run - which made a "seeded" run reproducible everywhere except the nibblers.
    // The local shuffle draws from Random.get, which is the same stream getRandomSpawns uses.
    const nibblerSpawns = InfernoWaves.shuffle([
      { x: 8 + 11, y: 13 + 14 },
      { x: 9 + 11, y: 13 + 14 },
      { x: 10 + 11, y: 13 + 14 },
      { x: 8 + 11, y: 12 + 14 },
